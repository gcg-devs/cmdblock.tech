...
#### Scenario: Render with payment protocol and QR code
- **WHEN** the HTML template is rendered and a payment protocol with `qr_data` is available
- **THEN** the footer shows the payment protocol label, bank name, account name, account number, and a branded QR code displayed beside the protocol details

#### Scenario: Render with payment protocol without QR code
- **WHEN** the HTML template is rendered and a payment protocol without `qr_data` is available
- **THEN** the footer shows the payment protocol details without a QR code
